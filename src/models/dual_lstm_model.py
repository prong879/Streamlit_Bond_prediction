...
def train_dual_lstm_model(x_train, y_train, input_dim, hidden_dim=32, num_layers=2, output_dim=1, 
    ...
    # 创建模型
    model = DualLSTM(input_dim=input_dim, hidden_dim=hidden_dim, output_dim=output_dim, num_layers=num_layers)
    ...
    counter = 0
    early_stop = False
    best_model_path = os.path.join(output_dir, 'dual_lstm_best_model.pth') if output_dir else 'dual_lstm_best_model.pth'
